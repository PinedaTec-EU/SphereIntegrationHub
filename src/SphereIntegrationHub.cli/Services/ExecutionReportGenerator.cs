...
.bar-latency-green{background:#22c55e}
.bar-latency-amber{background:#f59e0b}
.bar-latency-red{background:#ef4444}
...
function latencyBarCls(stage) {
  const latency = stage.Latency;
  if (!latency) return '';
  const color = (latency.Color || latency.BandName || '').toLowerCase().replace(/[^a-z0-9]+/g, '-');
  return color ? 'bar-latency-' + color : '';
}
function barCls(stage) {
  ...
  const latencyCls = latencyBarCls(stage);
  if (latencyCls) return latencyCls;
